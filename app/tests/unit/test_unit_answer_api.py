...
def test_create_answer_valid(
    ...
    with patch("app.services.answer.AnswerService.create_answer") as mock_service:
        ...
        response = client.post(
            f"/questions/{sample_question.id}/answers/", json=answer_data
        )
...
def test_create_answer_empty_datetime(
    client: TestClient, sample_question: Question, answer_data: dict
) -> None:
...
def test_create_answer_invalid_endpoint(
    ...
    with patch("app.services.answer.AnswerService.create_answer") as mock_service:
        ...
        response = client.post(
            f"/questions_wrong/{sample_question.id}/answers/", json=answer_data
        )
