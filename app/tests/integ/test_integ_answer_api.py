...
def test_create_answer_valid(client: TestClient, sample_question: dict,
    ...
    response = client.post(
            f"/questions/{id_question}/answers/",
            json=answer_data
        )
...
def test_create_answer_invalid_question_id(client: TestClient,
                                           answer_data: dict) -> None:
...
def test_create_answer_nonexistent_question(client: TestClient,
                                            answer_data: dict) -> None:
...
def test_create_answer_invalid_endpoint(client: TestClient,
                                        sample_question: dict,
                                        answer_data: dict) -> None:
    """Тест на создание ответа по неверному эндпоинту."""
    id_question = sample_question["id"]
    response = client.post(
            f"/questions_wrong/{id_question}/answers/",
            json=answer_data
        )
    assert response.status_code == 404
